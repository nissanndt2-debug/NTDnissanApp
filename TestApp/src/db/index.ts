...
async function migrate(db: SQLite.SQLiteDatabase): Promise<void> {
  ...
  // En una base nueva la columna ya viene incluida en DDL; ALTER solo aplica
  // a instalaciones existentes que todavia estaban en la versión 1.
  if (current >= 1 && current < 2) {
    await db.execAsync("ALTER TABLE defect ADD COLUMN photo_error TEXT;");
  }
